...
export async function activateHotspotUser({
  ...
  try {
    ...
    try {
      ...
      if (addError.message && addError.message.includes("already have user")) {
        ...
        if (existingUser) {
          ...
          // CRITICAL: Reset counters for returning users
          // Without this, uptime from previous session prevents new login
          try {
    ...
    try {
      // CRITICAL FIX: Do NOT query bindings - it causes uncatchable !empty errors
      // Instead, try to add directly and handle "already exists" error
      try {
        ...
        if (
          ...
          (addBindingError.message.includes("already have") ||
            addBindingError.message.includes("entry with the same"))
        ) {
          console.log(`ℹ️ IP binding already exists, will update it...`);
          ...
          try {
            ...
            // If print fails with !empty, it means table is empty somehow
            // This shouldn't happen since we just got "already exists" error
            // But if it does, just continue - the binding might exist anyway
            console.log(
              `⚠️ Could not list bindings after 'already exists' error - continuing anyway`
            );
            throw addBindingError; // Re-throw to trigger outer catch
          }
          ...
          // Some other error - re-throw to trigger outer catch

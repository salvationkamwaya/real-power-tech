...
export async function activateHotspotUser({
  ...
  try {
    ...
    try {
      ...
      // NOTE: Even getting all bindings can cause !empty if table is completely empty
      // Wrap everything in try-catch to handle this
      let existingBindings = [];
      let bindingsQueried = false;
      ...
      try {
        ...
        if (Array.isArray(allBindings) && allBindings.length > 0) {
        ...
        // Handle !empty error - this can happen if binding table is completely empty
        // or if there's any query issue
        console.log(
          `ℹ️ Could not query IP bindings (likely empty table): ${queryError.message}`
        );
        // Continue to create new binding
      }

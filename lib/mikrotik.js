...
export async function activateHotspotUser({
  ...
  try {
    ...
    if (existingUsers && existingUsers.length > 0) {
      // User exists - update the uptime limit and comment
      const existingUserId = existingUsers[0][".id"];
      console.log(`⚠️ User already exists (ID: ${existingUserId}), updating instead...`);
    ...
    // Add IP binding to bypass authentication for this MAC
    // This makes the user automatically authenticated when they connect

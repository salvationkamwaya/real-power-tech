  ...
  try {
    ...
    try {
      ...
      if (addError.message && addError.message.includes("already have user")) {
        ...
        try {
          // CRITICAL: Get ALL active sessions without filter to avoid !empty error
          // Then filter in JavaScript to find user's session
          let activeSessions = [];
          try {
            ...
            // If print fails with !empty, it means no active sessions exist
            // This is expected if session expired - just continue
            if (printError.message && printError.message.includes("!empty")) {
              console.log(
                `ℹ️ No active sessions found (table empty - expected if session expired)`
              );
            } else {
              console.warn(
                `⚠️ Could not list active sessions:`,
                printError.message
              );
            }
          }
          ...
          if (activeSessions && activeSessions.length > 0) {
            // Find and remove user's active sessions
            for (const session of activeSessions) {
              if (session.user === mac) {
                ...
                console.log(
                  `✅ Removed stale active session: ${session[".id"]}`
                );
              }
            }
          } else {
            console.log(
              `ℹ️ No active sessions found for user (expected if session expired)`
            );
          }
        } catch (sessionError) {
          console.warn(
            `⚠️ Could not remove active sessions:`,
        ...
        if (existingUser) {
          ...
          console.log(
            `✅ MikroTik user updated successfully (ready for new session):`,
            userId
          );
        } else {
          console.log(
